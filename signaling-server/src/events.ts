...
//
// FIXES:
// [TS] Removed RTCSessionDescriptionInit and RTCIceCandidateInit type
//      annotations. These are browser-only WebRTC types; Node.js has no
//      WebRTC runtime and ts-node cannot compile them (TS2304).
//      The signaling server never inspects SDP or ICE content — it just
//      relays the objects between peers — so `any` is the correct type here.
...
export const setupSocketEvents = (io: Server) => {
  io.on('connection', (socket: Socket) => {
    ...
    // ── 2. Screen-share offer (host → viewer) ───────────────────────────────
    // signalData is an SDP object — relayed as-is, never parsed on the server
    socket.on('call-user', (data: {
      userToCall: string;
      from: string;
      signalData: any;       // RTCSessionDescriptionInit — browser type only
    }) => {
      const { userToCall, signalData, from } = data;
      if (!isValidSDP(signalData)) {
        console.warn(`[SDP] invalid offer from ${socket.id}`);
    ...
    socket.on('answer-call', (data: {
      to: string;
      signal: any;           // RTCSessionDescriptionInit — browser type only
    }) => {
      const { to, signal } = data;
      if (!isValidSDP(signal)) {
        console.warn(`[SDP] invalid answer from ${socket.id}`);
    ...
    socket.on('ice-candidate', (data: {
      target: string;
      candidate: any;        // RTCIceCandidateInit — browser type only
    }) => {
      const { target, candidate } = data;
      if (!isValidCandidate(candidate)) return;
      io.to(target).emit('ice-candidate', { candidate, from: socket.id });
    ...
    // ── 6. Host acknowledges viewer join ────────────────────────────────────
    socket.on('viewer-pending-ack', (data: { to: string }) => {
      if (!data?.to) return;
      console.log(`[accept] pending ack: ${socket.id} → ${data.to}`);
      io.to(data.to).emit('waiting-for-host');
